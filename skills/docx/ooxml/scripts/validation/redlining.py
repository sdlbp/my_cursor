...
class RedliningValidator:
    ...
    def validate(self):
        ...
        # First, check if there are any tracked changes by Agent to validate
        try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if self.verbose:
                print("PASSED - All changes by Agent are properly tracked")
            return True

    def _generate_detailed_diff(self, original_text, modified_text):
        """Generate detailed word-level differences using git word diff."""
        error_parts = [
            "FAILED - Document text doesn't match after removing Agent's tracked changes",
            "",
            "Likely causes:",
